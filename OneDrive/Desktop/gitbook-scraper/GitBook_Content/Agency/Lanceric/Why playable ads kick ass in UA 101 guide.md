...
Actions speak louder than words. Doing is always better than showing. Don’t talk about buying your wife flowers. Buy them.

It’s the same with playable ads. Why show what your game is about when you can get people to play it? Video ads get boring too, and playable ads can add some spice to your UA campaigns. For the unfamiliar, playable ads are minigames that give users a brief glimpse of what your game looks like.

Here’s everything you need to know about them.

## Before we start, what to expect from creation and distribution of your playable ads?

Before we start, let’s set some expectations.

1. **Playable Ads are not as easy to create as a static image.** Do not expect to create a complicated brief and than be angry that your ad takes long to develop and is expensive and you need to export lot of assets.
2. **Playable Ads don’t have to copy your game.** So you use 30 AI generated videos and 100 AI generated images, with all different hooks that work but are irrelevant to your game, but for playables you want 100% gameplay? See where I am going here? Use that open mind in Playable Ads as well.
3. **3D games = 3D playables = bigger budgets.** Playable Ads need to be optimized carefully to fit requirements and with 3D games, it is no fun. Or use similar approach as in previous point – be open minded and focus on the theme rather than achieving 100% original gameplay.
4. You found a contractor for you playable ad, you are taking 3 weeks to approve concept and budget, 2 weeks to gather all assets(and still some missing), yet you expect your contractor to start working tomorrow and deliver on time. Understand companies that develop custom playables need your hand mostly in the beginning, **if you do not give the right tools at the right time, you are the bottleneck.**
5. **One playable ads is NOT all you need.** Those days are gone, when you developed a perfectly nice looking playable, put it in your campaign and magically you were winning. Playables are another part of your campaign mix, and same as videos, images, playables need to be produced at scale, compared, iterated and produced again.

Now you are ready to learn more, enjoy the reading!
...
**They’re cost-friendly to run.** On Android, playables are one of the cheapest to run on [($0.60)](https://content.liftoff.io/hubfs/2024/Reports/Mobile%20Ad%20Creative%20Index/2024%20Mobile%20Ad%20Creative%20Index%20-%20EN.pdf) and run at less than half the cost of native ads ($1.22).
...
**There are more or less these versions of playable ads:**
...
* Interactive end cards made from both video or as html
* Combination of taps/and [interactions](https://playablemaker.com/types-of-playable-ads-to-create-for-every-campaign/) – one tap, video+end card, multiple interactions…

The video format is easier to produce and may be lighter weight, but the HTML5 version is often more visually appealing and accurate to the product and is responsive. Learn more about [types](https://playablemaker.com/types-of-playable-ads-to-create-for-every-campaign/).
...
* JavaScript plays a central role in playable ads. It typically uses the HTML <canvas> element to render the game. With the help of a JavaScript game engine, the game is drawn into the canvas, which in this case is set to fullscreen and acts as the main display for the playable. Note: The <canvas> element allows you to draw and render graphics directly. Note 2: While using a game engine isn’t strictly required, developing without one can be quite challenging (most developers prefer using one).
* For development, it’s best to use a JavaScript game engine. However, the engine’s size is crucial because playable ads have strict size limitations. For 2D game development, engines like PixiJS and Phaser are good options. For 3D games, Three.js and Babylon.js are noteworthy. While Babylon.js is excellent for development, it requires tree-shaking to fit within size limits. Tree-shaking involves extracting only the code used in your app (in the playable ad) from the library, reducing the final package size. However, not all libraries support tree-shaking, and it can be challenging, especially if you need multiple features from the library.

## MRAID 2.0

MRAID, short for Mobile Rich Media Ad Interface Definitions, is an industry-standard API developed by the Interactive Advertising Bureau (IAB) to simplify how rich media ads run inside mobile apps. It serves as a bridge between the ad creative (usually built in HTML5 and JavaScript) and the host app’s software development kit (SDK), enabling consistent interactions and ad behavior across different mobile environments.​

### What MRAID Does

MRAID allows rich media ads to perform standard actions—expand, resize, play videos, access device orientation, or open external links—without developers needing to code app-specific integrations. This “write once, run anywhere” standard ensures that interactive ad creatives can be displayed properly within any MRAID-compliant app, regardless of the mobile operating system or SDK.​

### Why MRAID 2.0 Is Required

Most modern ad networks require MRAID 2.0 support because it introduced essential features for full-screen, multi-part, and interstitial ad formats—critical functions for playable and interactive ads. While MRAID 1.0 covered basic expandable banners, version 2.0 added capabilities for video playback, orientation changes, and comprehensive ad life-cycle management. These are necessary for the seamless execution of playable ads, enabling users to engage with the ad like a mini-game before installation.​

### MRAID Implementation in Ad Networks

Playable ad creatives must comply with MRAID 2.0 standards to ensure compatibility with SDKs used by ad networks like ironSource, Unity, and AppLovin. Compliance means adhering to rules such as waiting for the `mraid.ready` event, using secure HTTPS assets, invoking methods like `mraid.open()` for redirects, and packaging creatives in a single HTML file. These rules guarantee smooth user experiences and standardized performance measurement across publishers.​

In short, MRAID 2.0 is the backbone of mobile playable ad delivery, allowing advertisers to build rich, engaging ad experiences that function uniformly across multiple ad platforms and app environments. This universalization saves developers time, reduces costs, and ensures consistent user engagement across devices.

​You can find powerful MRAID validator [here for free](https://tools.playablemaker.com/)
...
Non-programmers can easily create playable ads using drag-and-drop tools, like ADS Kit and 2DKit, without any coding knowledge. Alternatively, you can hire mobile ad developers to build custom ads for you.

At the moment, 2DKit is probably the best self-creation tool that has built-in visual scripting. They make it easy to export to the most popular ad networks. Luckily there’s a wealth of good YouTube tutorials on how to use the platform as well. Playablemaker and ADS Kit are strong competitors that have solid UI that caters to more non-coders.Although many things in 2DKit can be achieved using drag-and-drop (D&D), it’s highly unlikely that you won’t need to use scripts (JavaScript) to achieve the desired effects. It’s similar to **Unity**—while it’s possible to work without scripting, in reality, you’ll almost always end up needing to write code. So, one way or another, you’re back to coding.
...
Applovin is the holy grail, or holy mother of playables. You want to get results on Applovin? You NEED playable ads. They also offer great preview [tool](https://p.applov.in/playablePreview?create=1) – so you can easily check your playables before you use them.

**Meta (Facebook) Ads**

Facebook allows you to upload a HTML5 or zip file of your playable ads for Android and iOS placements. Meta offers a [partner directory](https://www.facebook.com/business/partner-directory/search?solution_type=creative_platform&capabilities=Playables) for agencies and companies that help create them as well. You can read more about the [specs](https://www.facebook.com/business/help/412951382532338?helpref=faq_content) here.

**Google Ads**

You can create HTML5 playable ads for App Campaigns on Google. You can build your own playable ad using Google Web Designer, although there are better 3rd-party tools out there. AMPHTML assets are not supported at the moment. And they are no eligible to serve on App Campaigns for pre-registration and App Campaigns for engagement. Here are [detailed instructions](https://support.google.com/google-ads/answer/9981650?hl=en) on how to upload your HTML5 assets on Google Ads.

**Unity Ads**

Unity’s ad network allows for HMTL5 playable ads. If you’re using Unity’s game engine, you can create playable ads with your own assets, but there are way better tools out there. When it comes to Unity, we need to mention **Luna**, a costly tool that helps create HTML5-compatible playables (it’s integrated with Unity: [Luna Labs Docs](https://docs.lunalabs.io/)). However, Unity itself has significant limitations, such as a strict **5MB size limit**. While Unity offers a **Web Build** feature using WebGL, it’s not suitable for playable ads because the resulting package is enormous and relies heavily on external assets.
...
TikTok encourages advertisers to experiment with playables. And like the other guys, they accept HTML5 playable ads with a file size under 5MB to improve performance and loading speeds. TikTok audience have one of the least forgiving attention spans, so this would be a real test for your engagement metrics. You can [read more here](https://ads.tiktok.com/help/article/playable-ads?lang=en).
...
**Completion rate:** how many users finished playing the ad. This indicates how good the ad is at engaging the user and holding their attention.
...
**Keep gameplay simple.** Users should immediately understand how to interact with the ad. You’ve got only seconds to hook new players. Add a challenge for a sense of reward, but don’t overcomplicate it. For hypercasual games, keep it under 15 seconds, and under 28 seconds for more complicated games.
...
**Combine playables with other ad formats**. Consider showing a playable after a triple-page ad or after a short video.

**Show progress** with visual progress bars, racking up points, leaderboards, or cues that indicate milestones motivate users to complete the game. The psychology of [points systems](https://theirf.org/research_post/the-psychology-of-points/#:~:text=Perceptions%20of%20Points%20Programs%3A%20The%20Psychology%20of%20Points&text=First%2C%20we%20found%20reported%20satisfaction,than%20those%20who%20do%20not.), is one that mobile games take advantage of all the time, so why not use it here?
...
One of the oldest plays in the book is to ask players to “play your game” but just send you to the App Store as soon as you click on it. Some ads have fake close (x) buttons or tiny ones that make you accidentally click through to the app store to boost CTR metrics. Don’t be Gardenscapes.

2. **Low-quality apps can’t be saved**
...
3. **Don’t bother with apps with small target audiences**

Unless you have an unlimited budget, it’s not worth investing in developing playable ads for apps with small audiences. Unless you use no-code tools where you can create lots of playables easily and ABtest before making a decision to go full on custom playables.

### **References**

## Playable Brief example: Unleash Magic and Defend the Realm!
...
**Text Overlay:** “Collect heroes, upgrade spells, and defend the realm! Download *AFK Magic TD* now and unleash the magic within!”
...
* Day 1: “I will just export and share this 359000 TB of game assets to the developer and he will pick something”
...
* The fonts and some assets can be messy on an external display.
* The best way to test out the game is to use a test app from your target Ad Network.
* Maybe the easiest way is to use AppLovin Playable Preview. https://p.applov.in/playablePreview?create=1&qr=1
* We recommend to test it on phone (there is a link to download the AppLovin test app, then scan the QR code with the app)
